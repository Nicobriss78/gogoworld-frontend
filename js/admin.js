...
function getToken() {
  // 1) sessionStorage: nomi piu comuni
  try {
    const direct =
...
elEvList.innerHTML = "";
  try {
   ...
   // Dedup per _id/id: evita duplicati quando il filtro matcha piu campi lato BE
...
const elUsLimit = document.getElementById("usLimit");
...
const seen = new Set();
const uniq = [];
  for (const u of (out && out.users ? out.users : [])) {
...
const base = apiBase();
const filters = readUserFiltersFromUI();
elUsLimit?.addEventListener("change", () => { usLimit = parseInt(elUsLimit.value, 10) || 20; usPage = 1; loadUsers(); });
...
[elUsSearch, elUsRole, elUsOrg, elUsBan].forEach(el => {
  el?.addEventListener("change", () => { usPage = 1; saveUserFilters(readUserFiltersFromUI()); loadUsers(); });
  el?.addEventListener("keyup", (e) => { if (e.key === "Enter") { usPage = 1; saveUserFilters(readUserFiltersFromUI()); loadUsers(); } });
});
